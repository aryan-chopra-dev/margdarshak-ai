...
export interface University {
  ...
  country: string;
  city: string;
  type: 'abroad' | 'domestic';
  qsRank2025: number; // QS World University Rankings 2025
  programs: string[];
  tuitionUSD: number;         // Annual tuition for international students
  avgNetPriceUSD: number;     // Average net price after aid (US DOE data where available)
  admissionRate: number;      // Decimal (0.04 = 4%)
  medianEarnings10yr: number; // Median earnings 10 years after entry (US DOE)
  studentSize: number;
  tags: string[];
  dataSource: string;
  sourceUrl: string;
