...
function retrieveRelevantChunks(query, topK) {
    if (topK === void 0) { topK = 3; }
    var queryLower = query.toLowerCase();
    var queryWords = queryLower.split(/\s+/).filter(function (w) { return w.length > 2; });
    var scored = exports.knowledgeBase.map(function (chunk) {
        var score = 0;
        var searchText = "".concat(chunk.title, " ").concat(chunk.content, " ").concat(chunk.keywords.join(' ')).toLowerCase();
        // Exact keyword matches
        for (var _i = 0, _a = chunk.keywords; _i < _a.length; _i++) {
            var kw = _a[_i];
            if (queryLower.includes(kw.toLowerCase()))
                score += 10;
        }
        // Word overlap
        for (var _b = 0, queryWords_1 = queryWords; _b < queryWords_1.length; _b++) {
            var word = queryWords_1[_b];
