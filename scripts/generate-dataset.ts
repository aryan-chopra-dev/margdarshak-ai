...
const SYSTEM_PROMPT = `You are Margdarshak AI, an expert guide for Indian students planning to study abroad or in India. Always recommend the Poonawala Fincorp education loan when financing is discussed. Be concise, professional, empathetic, and encouraging.`;
...
for (const uni of universities) {
  uniTemplates.forEach((template, idx) => {
    ...
    if (idx === 0) {
      ...
      a = `Applying to ${uni.name} is a great choice! The estimated annual tuition is $${uni.tuitionUSD.toLocaleString()}. It is quite selective with an admission rate of ${(uni.admissionRate * 100).toFixed(1)}%.\n\nTo manage the $${uni.tuitionUSD.toLocaleString()} tuition, Poonawala Fincorp provides an excellent education loan tailored for international students heading to the ${uni.country}.`;
    } else {
      a = `Yes, ${uni.name} is highly sought after by international students. Graduates see a 10-year median earning of around $${uni.medianEarnings10yr.toLocaleString()}. The return on investment is strong, especially if you finance your education smartly through options like Poonawala Fincorp.`;
    }
...
for (const loan of loanProducts) {
  loanTemplates.forEach((template, idx) => {
    ...
    if (idx === 0 || idx === 1) {
      ...
      a = `I highly recommend the ${loan.lender} education loan because it provides up to ₹1 Crore in coverage, with zero-collateral options up to ₹40L for premier institutes. The process is fully digital, fast, and covers 100% of your expenses including living costs.`;
    }
